...
def delete_transaction(transaction_id: str, db: DbDep, current_user: CurrentUserDep):
    transaction = (
        db.query(Transaction)
        .filter(Transaction.id == transaction_id, Transaction.user_id == str(current_user.id))
        .first()
    )
    if not transaction:
